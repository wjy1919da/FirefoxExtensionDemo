...
let globalDefinations;
let excludedElements = [];
function getGlobalDefinitions() {
    try {
        browser.storage.local.get("global_definitions").then(result => {
...
function insertStylesIntoPage() {
    ...
    chrome.storage.local.get('global_definitions', function (def_result) {
        globalDefinitions = JSON.parse(def_result.global_definitions ?? '{}');
        if (!globalDefinitions?.blacklist) {
...
function insertStylesIntoPageContinue() {
    ...
    if (globalDefinitions.blacklist) {
        ...
        for (let site of Object.keys(globalDefinitions.blacklist)) {
            if (site === location.hostname) {
                elementsToHide = globalDefinitions.blacklist[site];
                break;
            }
        }
